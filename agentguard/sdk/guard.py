...
class Guard:
    ...
    def attach_claude_agent(self, options: Any) -> Any:
        """Attach AgentGuard to a Claude Agent SDK ``ClaudeAgentOptions``.

        Mutates the options in place:

        * For each SDK MCP server in ``options.mcp_servers``, the registered
          ``@tool`` callables are wrapped with guarded variants.
        * A ``PreToolUse`` hook covering Claude Code built-in tools (Bash,
          Read, Write, Edit, WebFetch, ...) is appended to ``options.hooks``.

        Returns the adapter instance for inspection.
        """
        from agentguard.sdk.adapters.claude_agent import ClaudeAgentAdapter
        adapter = ClaudeAgentAdapter(self.pipeline, self)
